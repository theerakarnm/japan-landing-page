...
const WhatNewElem = ({ data }: Props) => {
  return (
    <Link
      href={data.linkTo ? data.linkTo : `/WhatNewPage/${data.id}`}
      className='hover:text-[#3A7C4E]'>
      <a>
        <div className='border-b-2 border-dotted border-[#3A7C4E] my-5 py-2'>
          ...
          <p className='font-normal text-gray-600 text:sm md:text-md'>{data.description}</p>
        </div>
      </a>
    </Link>
  );
